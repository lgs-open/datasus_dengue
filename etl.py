...
def download_file(path_download: str, file_name: str) -> None:  
    ...
    global PATH_DIR_RAW
    try:
        #dir_raw = 'data\\raw' # diretório dos arquivos sem tratamento
        #path_dir = os.path.join(os.getcwd(), dir_raw) #caminho da pasta!!!
...
def pedido(linha: list, coluna: list, estado:str, ano: int) -> None:
     '''Cria a variável de solicitação, o caminho de solicitação, faz o pedido e cria o arquivo tipado.'''
...
linhas = [['Munic%EDpio_de_notifica%E7%E3o', 'municipio_notificacao'],
...
coluna= ["M%EAs_notifica%E7%E3o","mes_notificacao"]



# Gerando os arquivos de 2014 a 2015 com as linhas e colunas acima para o estado do Paraná
for ano in range(14,25):
    for linha in linhas:
        pedido(linha, coluna, dic_estados['parana'], ano)
